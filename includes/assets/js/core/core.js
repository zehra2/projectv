...
$(function () {
  ...
  $('body').on('submit', '.js_search-form', function (e) {
    ...
    var qry_country = $('#search-form-country-selector').find(":selected").val();
    var qry_state = $('#search-form-state-selector').find(":selected").val();
    var qry_city = $('#search-form-city-selector').find(":selected").val();

    if(is_empty(qry_country) || is_empty(qry_state) || is_empty(qry_city)){
      return false;
    }
    ...
    if(!is_empty(qry_country) && !is_empty(qry_state) && !is_empty(qry_city)){
      if (handle !== undefined) { 
        window.location = site_path + '/' + handle + '/search?country=' + qry_country+'&state='+qry_state+'&city='+qry_city;
      }
    } 
