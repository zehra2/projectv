...
class User
{
  ...
  public function get_countries()
  {
    global $db, $system;
    $countries = [];
    $get_countries = $db->query("SELECT * FROM countries ORDER BY id ASC") or _error('SQL_ERROR_THROWEN');
    if ($get_countries->num_rows > 0) {
      while ($country = $get_countries->fetch_assoc()) {
        $country['country_name_native'] = $country['code'];
        $country['country_name'] = __($country['name']);
        $countries[] = $country;
      }
    }
    return $countries;
  ...
  public function publisher($args = [])
  {
    ...
    switch ($post['post_type']) {
      ...
      case 'product':
        ...
        $db->query(sprintf("INSERT INTO posts_products (post_id, name, price, quantity, category_id, status, location, country, state, city, mediatype, facebook_url, youtube_url, marketplace_type, product_size, available_unit, sub_category_id ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", secure($post['post_id'], 'int'), secure($args['product']->name), secure($args['product']->price, 'float'), secure($args['product']->quantity), secure($args['product']->category, 'int'), secure($args['product']->status), secure($args['product']->location),secure($args['product']->country), secure($args['product']->state), secure($args['product']->city), secure($args['product']->mediatype), secure($args['product']->facebookurl), secure($args['product']->youtubeurl), secure($args['product']->marketplace_type), secure($args['product']->product_size), secure($args['product']->available_unit), secure($args['product']->sub_category))) or _error('SQL_ERROR_THROWEN'); 
    ...
    if (!$post['needs_subscription'] || !$post['is_paid']) {
      ...
      switch ($post['post_type']) {
        ...
        case 'product':
          ...
          if($post['product']['category_id']){
            $get_category = $db->query("SELECT * FROM i_profile_categories WHERE c_id =".$post['product']['category_id']."");

            if ($get_category->num_rows > 0) {
              $category = $get_category->fetch_assoc();
              $category_name = $category['c_key'];
             
            }
            
          }
          if(!empty($category_name)){
            $post['product']['category'] = $category_name;
          }else{
            $post['product']['category'] = '';
          }
          
          /* Fetch contry, state and city based on id */
          if($post['product']['country'] || $post['product']['state'] || $post['product']['city']){
            $get_country = $db->query("SELECT * FROM countries WHERE id =".$post['product']['country']."");
            
            $get_state = $db->query("SELECT * FROM states WHERE id =".$post['product']['state']."");
            
            
            $get_city = $db->query("SELECT * FROM cities WHERE id =".$post['product']['city']."");
            ...
            if ($get_state->num_rows > 0) {
              $state = $get_state->fetch_assoc();
              $state = $state['name'];
            }
            if ($get_city->num_rows > 0) {
              $city = $get_city->fetch_assoc();
              $city = $city['name'];
            }
            
            $post['product']['full_address'] =  $country.', '.$state.', '.$city;
  ...
  public function edit_product($post_id, $message, $args = [])
  {
    ...
    /* update product */
    $db->query(sprintf("UPDATE posts_products SET name = %s, price = %s, quantity = %s, category_id = %s, status = %s, location = %s, country = %s, state = %s, city = %s, mediatype = %s, facebook_url = %s, youtube_url = %s, marketplace_type = %s, product_size = %s, available_unit = %s, sub_category_id = %s WHERE post_id = %s", secure($args['name']), secure($args['price'], 'float'), secure($args['quantity']), secure($args['category'], 'int'), secure($args['status']), secure($args['location']), secure($args['country']), secure($args['state']), secure($args['city']), secure($args['mediatype']), secure($args['facebookurl']), secure($args['youtubeurl']), secure($args['marketplace_type']), secure($args['product_size']), secure($args['available_unit']), secure($args['sub_category']), secure($post_id, 'int'))) or _error('SQL_ERROR_THROWEN');
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      case 'account':
        ...
        if (sha1(md5(trim($args['password']))) != $this->_data['user_password'] && !password_verify($args['password'], $this->_data['user_password'])) {
      ...
      case 'password':
        ...
        /* validate current password (MD5 check for versions < v2.5) */
        if (sha1(md5(trim($args['current']))) != $this->_data['user_password'] && !password_verify($args['current'], $this->_data['user_password'])) {
          throw new Exception(__("Your current password is incorrect"));
        }
  ...
  /**
   * Fetch the states based on  country id
   * 
   */
  
  public function get_states_by_country($args = [])
  {
    global $db, $system;
    $states = [];

    $query = $db->query(sprintf("SELECT * FROM posts_products WHERE post_id = %s", secure($args['node_id']))) or _error('SQL_ERROR_THROWEN');
    if ($query->num_rows > 0) {
      ...
      if(!empty($post)){
        ...
        $states_qry = $db->query(sprintf("SELECT * FROM states WHERE country_id = %s", secure($post['country']))) or _error('SQL_ERROR_THROWEN');

       
        if ($states_qry->num_rows > 0) {
            
            while ($state = $states_qry->fetch_assoc()) {

              $state['id'] = $state['id'];
              $state['state_name'] = __($state['name']);
              $state['code'] = __($state['code']);
              $states[] = $state;
   ...
   * 
   */
  public function get_cities_by_state($args = [])
  {
    global $db, $system;
    $cities = [];
    ...
    if ($query->num_rows > 0) {
      ...
      if(!empty($post)){
        ...
        $cities_qry = $db->query(sprintf("SELECT * FROM cities WHERE state_id = %s", secure($post['state']))) or _error('SQL_ERROR_THROWEN');

       
        if ($cities_qry->num_rows > 0) {
            
            while ($city = $cities_qry->fetch_assoc()) {

              $city['id'] = $city['id'];
              $city['city_name'] = __($city['name']);
              $cities[] = $city;
  ...
  public function get_product_categories($args = [])
  {
    ...
    $get_categories = $db->query("SELECT * FROM i_profile_categories WHERE c_status = '1'") or _error('SQL_ERROR_THROWEN');

    if ($get_categories->num_rows > 0) {
      while ($category = $get_categories->fetch_assoc()) {
        $category['category_id'] = $category['c_id'];
        $category['category_name'] = __($category['c_key']);
        $category['category_status'] = $category['c_status'];
        $categories[] = $category;
      }
    } 
    return $categories;
  ...
  /**
   * Fetch the product sub categories by category ID
   * 
   */
  public function get_sub_category_by_category_id($args = [])
  {
    global $db, $system;
    $sub_categories = [];
    ...
    if ($query->num_rows > 0) {
      ...
      if(!empty($post)){
        ...
        $sub_cat_qry = $db->query(sprintf("SELECT * FROM i_profile_sub_categories WHERE c_fk = %s", secure($post['category_id']))) or _error('SQL_ERROR_THROWEN');

        
        if ($sub_cat_qry->num_rows > 0) {
            
            while ($sub_category = $sub_cat_qry->fetch_assoc()) {

              $sub_category['sub_category_id'] = $sub_category['sc_id'];
              $sub_category['sub_category_name'] = __($sub_category['sc_key']);
  ...
  public function get_country_name_by_id($country_id)
  {
    ...
    $get_country = $db->query(sprintf("SELECT * FROM countries WHERE id = %s", secure($country_id))) or _error('SQL_ERROR_THROWEN');
  ...
  public function get_state_name_by_id($state_id)
  {
    ...
    $get_state = $db->query(sprintf("SELECT * FROM states WHERE id = %s", secure($state_id))) or _error('SQL_ERROR_THROWEN');
  ...
  public function get_city_name_by_id($city_id)
  {
    ...
    $get_city = $db->query(sprintf("SELECT * FROM cities WHERE id = %s", secure($city_id))) or _error('SQL_ERROR_THROWEN');
