...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'search':
      ...
      // check query
      /*if (!isset($_GET['query']) || is_empty($_GET['query']) || $_GET['country']) {
        redirect('/market');
      }*/
      ...
      if(!is_empty($_GET['country']) && !is_empty($_GET['state']) && !is_empty($_GET['city'])){
        
        $country_name = $user->get_country_name_by_id($_GET['country']);
        $state_name = $user->get_state_name_by_id($_GET['state']);
        $city_name = $user->get_city_name_by_id($_GET['city']);

        $smarty->assign('query', $country_name.','.$state_name.','.$city_name);
        $smarty->assign('qry_country', $_GET['country']);
        $smarty->assign('qry_city', $_GET['city']);
        $smarty->assign('qry_state', $_GET['state']);
        
        $where_query = sprintf('AND (posts_products.country = '.$_GET['country'].') AND (posts_products.state = '.$_GET['state'].') AND (posts_products.city = '.$_GET['city'].')');
